...
def training_loop(model, nr_epochs, path_images, training_type, steps_per_epoch = (6000 * 2/4), validation_steps = (6000 * 1/4)):
    ...
    for epoch in tqdm.tqdm(range(nr_epochs), total = nr_epochs):
        ...
        logging.info(f"starting {training_type} epoch #{epoch}")
        history=model.fit_generator(training_gen, steps_per_epoch = steps_per_epoch, epochs = 1,
                                    validation_data = validation_gen, validation_steps = validation_steps)
        logging.info(f"finished {training_type} epoch #{epoch}")

        model.save(Path(path_checkpoints) / f'{training_type}_{epoch}.checkpoint')

        training_error, validation_error=1. -
            history.history['acc'][0], 1. - history.history['val_acc'][0]
        training_loss, validation_loss=history.history['loss'][0], history.history['val_loss'][0]

        write_status(Path(path_statuses) / f'{training_type}_{epoch}.status', epoch,
                                                                              training_error,
                                                                              validation_error,
                                                                              training_loss,
                                                                              validation_loss)
...
def main_training(path_images, path_checkpoints, path_statuses, current_epoch, nr_epochs=100000):
    ...
    current_checkpoint_filename = Path(path_checkpoints) / f'main_{current_epoch}.checkpoint'
...
if __name__ == '__main__':
    ...
    logging.info(("started with parameters",
                 f"path_images: '{path_images}''",
                 f"path_checkpoints: '{path_checkpoints}''",
                 f"mode: '{mode}''"))
